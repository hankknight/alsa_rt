...
namespace alsa_rt
{
    ...
    PlayBackPriorityStreamDebug::PlayBackPriorityStreamDebug(AlsaConfig cfg) : PlayBackPriorityStream(cfg) {}

    void PlayBackPriorityStreamDebug::loop()
    {
        ...
        while (running_)
        {
            ...
            if (target_pos > 0)
            {
                ...
                s = "[DEBUG] priority flush: skip_to=" + std::to_string(target_pos) +
                    ", current_priority=" + std::to_string(current_priority_.load(std::memory_order_acquire));
                log(LogLevel::Info, s);
            }
            ...
            if (to_read < period_bytes)
            {
                ...
                {
                    ++starvations;
                    current_priority_.store(0, std::memory_order_release);
    ...
    void PlayBackPriorityStreamDebug::onDumpPcmState()
    {
